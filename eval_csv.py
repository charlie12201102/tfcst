...
#计算Recall、precision、mAP
def evaluate(model, data_loader, save_path):
    ...
    FPS = num / cost_time
    print('Total Frame:%d,  cost %f s' % (num, cost_time))
    ...
    recordLine = '  F1: ' + str(F1_score) + '  Precision: ' + str(precision) + '  Recall: ' + str(recall) +'  FPS: ' + str(FPS)+ '\n'
    ff.write(recordLine)
    return F1_score


#评估单帧检测器的性能：包含检测率、虚警率、FPS
def main():
    #运行命令中参数解析
    ...
    parser.add_argument('--annot_file', default='datasets/Indices/new_test_less.txt',
                        help='Path to file containing evaliating annotations')
    parser.add_argument('--model_path', default='checkpoint/rnn/TinyVoVNet_320_ASFF_Pretian.pth',help='Path to model checkpoint')
    opt = parser.parse_args()

    #单帧图像测试集的数据加载
    eval_dataset = CSVDataset(annot_file=opt.annot_file, resize_size=[320, 320], is_train=True, augment=None)

    print("Evaluatintn datasets")
    eval_loader = torch.utils.data.DataLoader(eval_dataset, batch_size=opt.batch_size, shuffle=False,collate_fn=eval_dataset.collate_fn)
    ...
    #评估，并将结果记录在record/test.txt中
    evaluate(model, eval_loader, 'record/test.txt')
