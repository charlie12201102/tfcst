...
from .backbone.vovnet import vovnet_tiny, vovnet27_slim, vovnet39, vovnet57, vovnet_naon
from .head import ClsCntRegHead
from .fpn_neck import FPN, ASFF_FPN, FPN_FRM
...
class FCOS(nn.Module):
    ...
    def __init__(self, config=None):
        ...
        # fpn_size = [64, 128, 256, 512]

        # self.backbone = resnet34(pretrained=config.pretrained)
        # fpn_size = [64, 128, 256, 512]

        # self.backbone = resnet50(pretrained=config.pretrained)
        # fpn_size = [256, 512, 1024, 2048]

        # self.backbone = darknet('nano',depthwise=True)
        # fpn_size = [32, 64, 128, 256]

        # self.backbone = darknet('tiny')
        # fpn_size = [48, 96, 192, 384]

        # self.backbone = darknet('s')
        # fpn_size = [64, 128, 256, 512]

        # self.backbone = vovnet27_slim()
        # fpn_size = [128, 256, 384, 512]

        # self.backbone = vovnet39()
        # fpn_size =[256, 512, 768, 1024]

        # self.backbone = vovnet57()
        # fpn_size = [256, 512, 768, 1024]

        #特征提取网络
        self.backbone = vovnet_tiny()
        #backbone输出的C1、C2、C3、C4输出通道数
        fpn_size = [64, 128, 192, 256]

        # self.backbone = vovnet_naon()
        # fpn_size = [48, 96, 144, 192]

        # 采用的特征融合方式
        # self.fpn = FPN(fpn_size, config.fpn_out_channels)
        # self.fpn = FPN_FRM(fpn_size, config.fpn_out_channels)
        self.fpn = ASFF_FPN(fpn_size, config.fpn_out_channels)

        self.head = ClsCntRegHead(config.fpn_out_channels, config.class_num, config.use_GN_head, config.cnt_on_reg,
                                  config.prior)

        # self.head = ClsRegIouHead(config.fpn_out_channels, config.class_num,config.use_GN_head, config.prior)
    ...
    def forward(self, x):
        ...
        # cls_logits, iou_preds, reg_preds = self.head(all_P)
        # return [cls_logits, iou_preds, reg_preds]
...
class DetectHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # iou_logits, _ = self._reshape_cat_out(inputs[1], self.strides)  # [batch_size,sum(_h*_w),1]
        ...
        if self.config.add_centerness:
            ...
            # cls_scores = torch.sqrt(cls_scores * (iou_preds.squeeze(dim=-1)))  # [batch_size,sum(_h*_w)]
