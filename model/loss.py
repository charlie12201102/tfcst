...
class GenTargets(nn.Module):
    def __init__(self, strides, limit_range,config=None):
        super().__init__()
    ...
    def _gen_level_targets(self, out, gt_boxes, classes, stride, limit_range):
        ...
        radiu = stride * self.config.sample_radiu_ratio
        ...
        reg_targets = ltrb_off[torch.zeros_like(areas).type(torch.uint8).scatter_(-1, areas_min_ind.unsqueeze(dim=-1),
                                                                                  1)]  # [batch_size*h*w,4]
        ...
        cls_targets = classes[
            torch.zeros_like(areas).type(torch.uint8).scatter_(-1, areas_min_ind.unsqueeze(dim=-1), 1)]
...
def compute_iou_loss(preds, targets, mask):
    '''
    Args
    preds: list contains five level pred [batch_size,1,_h,_w]
    targets: [batch_size,sum(_h*_w),1]
    ...
    loss = []
    for batch_index in range(batch_size):
        pred_pos = preds[batch_index][mask[batch_index]]  # [num_pos_b,1]
        target_pos = targets[batch_index][mask[batch_index]]  # [num_pos_b,1]
...
class LOSS(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.config.add_centerness:
            total_loss = cls_loss  + reg_loss + cnt_loss * self.config.lambda_ctn
...
#         self.past_score = None
#
#         for time_step, predictions in enumerate(scores):
#
#             output_score = Variable(torch.sum(input, dim=0, keepdim=True)[:, :, :, 0], requires_grad=False)
#
...
#
#         return loss_association / len(scores)
...
#         preds, targets = inputs
#
#         cls_logits, iou_logits, reg_preds = preds
#         cls_targets, iou_targets, reg_targets = targets
#
#         mask_pos = (iou_targets > -1).squeeze(dim=-1)  # [batch_size,sum(_h*_w)]
#         iou_loss = compute_iou_loss(iou_logits, iou_targets, mask_pos).mean()
#         cls_loss = compute_cls_loss(cls_logits, cls_targets, mask_pos).mean()  # []
#
#         reg_loss = compute_reg_loss(reg_preds, reg_targets, mask_pos).mean()
#
#         total_loss = cls_loss + reg_loss + iou_loss
#         return cls_loss, iou_loss, reg_loss, total_loss


if __name__ == "__main__":
    loss = compute_cnt_loss([torch.ones([2, 1, 4, 4])] * 5, torch.ones([2, 80, 1]),
                            torch.ones([2, 80]).type(torch.uint8))
    # loss = compute_cnt_loss([torch.ones([2, 1, 4, 4])] * 5, torch.ones([2, 80, 1]),
