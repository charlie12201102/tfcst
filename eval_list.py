...
def evaluate(model, datasets, save_path):
    ...
    for i in range(len(datasets)):
        ...
        for idx in range(len(seq_imgs)):
            ...
            with torch.no_grad():
                out_scores, out_classes, out_boxes, state = model(img.cuda(),state)
...
def main():
    ...
    parser.add_argument("--model_path", default='checkpoint/rnn/TinyVovnet_ASFF_320_ConvLSTM-14_best.pth')
    ...
    #评估，并将结果记录在txt文件中
    evaluate(model,eval_dataset,'record/evaluate_LSTM_Recall_Precision.txt')
